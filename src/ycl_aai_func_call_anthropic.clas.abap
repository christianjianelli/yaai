CLASS ycl_aai_func_call_anthropic DEFINITION
  PUBLIC
  CREATE PUBLIC.

  PUBLIC SECTION.

    INTERFACES yif_aai_func_call_anthropic.

    ALIASES on_tool_call          FOR yif_aai_func_call_anthropic~on_tool_call.
    ALIASES on_tool_call_response FOR yif_aai_func_call_anthropic~on_tool_call_response.
    ALIASES on_tool_call_error    FOR yif_aai_func_call_anthropic~on_tool_call_error.

    ALIASES mt_methods FOR yif_aai_func_call_anthropic~mt_methods.

    ALIASES add_methods FOR yif_aai_func_call_anthropic~add_methods.
    ALIASES get_tools FOR yif_aai_func_call_anthropic~get_tools.
    ALIASES reset_methods FOR yif_aai_func_call_anthropic~reset_methods.
    ALIASES remove_method FOR yif_aai_func_call_anthropic~remove_method.
    ALIASES call_tool FOR yif_aai_func_call_anthropic~call_tool.
...
CLASS ycl_aai_func_call_anthropic IMPLEMENTATION.

  METHOD yif_aai_func_call_anthropic~call_tool.
  ...
  METHOD yif_aai_func_call_anthropic~get_tools.

    DATA lt_tools TYPE STANDARD TABLE OF yif_aai_func_call_anthropic~ty_tool_s.
    ...
    LOOP AT me->mt_methods ASSIGNING FIELD-SYMBOL(<ls_method>).
      ...
      <ls_tool>-input_schema-type = 'object'.

      LOOP AT lt_importing_params ASSIGNING FIELD-SYMBOL(<ls_importing_param>).

        IF <ls_importing_param>-required = abap_true.

          APPEND <ls_importing_param>-name TO <ls_tool>-input_schema-required.

        ENDIF.

      ENDLOOP.

      <ls_tool>-input_schema-properties = lo_aai_util->get_json_schema(
        EXPORTING
          i_class_name  = <ls_method>-class_name
          i_method_name = <ls_method>-method_name
      ).
  ...
  METHOD yif_aai_func_call_anthropic~add_methods.
