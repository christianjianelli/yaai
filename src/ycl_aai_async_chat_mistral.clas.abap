CLASS ycl_aai_async_chat_mistral DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS ycl_aai_async_chat_mistral IMPLEMENTATION.
  ...
  METHOD yif_aai_async_chat_run~run.
    ...
    DATA(lo_aai_db) = NEW ycl_aai_db( i_api = yif_aai_const=>c_mistral
                                      i_id = i_chat_id ).
    ...
    DATA(lo_aai_openai) = NEW ycl_aai_openai( i_api = yif_aai_const=>c_mistral
                                              i_model = i_model
