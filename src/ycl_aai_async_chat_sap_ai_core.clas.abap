CLASS ycl_aai_async_chat_sap_ai_core DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    METHODS on_response_received FOR EVENT on_response_received OF ycl_aai_openai.

    METHODS on_message_failed FOR EVENT on_message_failed OF ycl_aai_openai
      IMPORTING
        error_text.

    METHODS on_chat_is_blocked FOR EVENT on_chat_is_blocked OF ycl_aai_openai.

    METHODS on_tool_call FOR EVENT on_tool_call OF yif_aai_func_call_openai
      IMPORTING
        class_name
        method_name.

    METHODS on_tool_call_response FOR EVENT on_tool_call_response OF yif_aai_func_call_openai
      IMPORTING
        tool_response.

    METHODS on_tool_call_error FOR EVENT on_tool_call_error OF yif_aai_func_call_openai
      IMPORTING
        error_text.
...
CLASS ycl_aai_async_chat_sap_ai_core IMPLEMENTATION.
  ...
  METHOD yif_aai_async_chat_run~run.
    ...
    DATA(lo_aai_conn) = NEW ycl_aai_conn( i_api = yif_aai_const=>c_sap_ai_core ).
    ...
    SELECT SINGLE low
      ...
      WHERE name = @yif_aai_const=>c_sap_ai_core_res_grp_param
        AND type = 'P'
        AND numb = '0000'
      INTO @DATA(l_resource_group).

    IF l_resource_group IS INITIAL.
      l_resource_group = yif_aai_const=>c_sap_ai_core_resource_group. "default
    ENDIF.

    lo_aai_conn->yif_aai_conn~add_http_header_param(
      EXPORTING
        i_name  = 'AI-Resource-Group'
        i_value = l_resource_group
    ).

    DATA(lo_aai_db) = NEW ycl_aai_db( i_api = yif_aai_const=>c_sap_ai_core
                                      i_id = i_chat_id ).
    ...
    DATA(lo_aai_openai) = NEW ycl_aai_openai( i_api = yif_aai_const=>c_sap_ai_core
                                              i_model = i_model
