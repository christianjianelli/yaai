CLASS ycl_aai_async_chat_google DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    METHODS on_message_send FOR EVENT on_message_send OF ycl_aai_google.

    METHODS on_response_received FOR EVENT on_response_received OF ycl_aai_google.

    METHODS on_message_failed FOR EVENT on_message_failed OF ycl_aai_google
      IMPORTING
        error_text.

    METHODS on_chat_is_blocked FOR EVENT on_chat_is_blocked OF ycl_aai_google.

    METHODS on_tool_call FOR EVENT on_tool_call OF yif_aai_func_call_google
      IMPORTING
        class_name
        method_name.

    METHODS on_tool_call_response FOR EVENT on_tool_call_response OF yif_aai_func_call_google
      IMPORTING
        tool_response.

    METHODS on_tool_call_error FOR EVENT on_tool_call_error OF yif_aai_func_call_google
      IMPORTING
        error_text.
...
CLASS ycl_aai_async_chat_google IMPLEMENTATION.
  ...
  METHOD yif_aai_async_chat_run~run.
    ...
    DATA(lo_aai_conn) = NEW ycl_aai_conn( i_api = yif_aai_const=>c_google ).

    lo_aai_conn->set_api_key( i_api_key = i_api_key ).

    DATA(lo_aai_db) = NEW ycl_aai_db( i_api = yif_aai_const=>c_google
                                        i_id = i_chat_id ).
    ...
    DATA(lo_aai_google) = NEW ycl_aai_google( i_model = i_model
