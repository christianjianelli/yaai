...
3. **Bind Tools to OpenAI**  
    The ABAP AI tools OpenAI `bind_tools` method expects an object (instance) of the class `ycl_aai_func_call_openai` as its argument. This object manages the registration and invocation of ABAP methods as callable tools for OpenAI function calling.
...
4. **Describe Functions to OpenAI**  
    The ABAP AI tools framework provides a JSON schema describing available methods and their parameters. This schema is sent to the OpenAI API as part of the function calling setup.
